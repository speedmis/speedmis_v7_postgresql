...
<?php
	...
	if($ActionFlag=="modify") {
		...
		$cnt_allPush_member = 1*onlyOnereturnSql("select count(*) from mis_group_members where group_idx=20 and user_id='$MisSession_UserID';");
...
function save_updateReady() {
    ...
    if($key_value!="") {
		if(isset($saveTextdecrypt2List["passwd_decrypt"])) {
        	$passwd_decrypt = $saveTextdecrypt2List["passwd_decrypt"];
			...
			$match1 = preg_match('/^[0-9]/u', $passwd_decrypt);
			$match2 = preg_match('/^(?!((?:[A-Za-z]+)|(?:[~!@#$%^&*()_+=]+)|(?:[0-9]+))$)[A-Za-z\d~!@#$%^&*()_+=]{6,12}$/u', $passwd_decrypt);
			$match3 = preg_match('/^.*(?=^.{6,12}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/u', $passwd_decrypt);
...
function save_updateAfter() {
	...
	if(request_cookies("my_language_code")!=$updateList['my_language_code']) {
		$afterScript = 'location.href = location.href;';
	}	
	setcookie('my_language_code', '', -1, '/', '.' . $base_domain);
	setcookie('my_language_code', $updateList['my_language_code'], time() + 3600*24*100, '/');
