...
function rowFunctionAfter_UserDefine(p_this) {
    ...
    if(p_this.AutoGubun.length==20) {
        $(getCellObj_idx(p_this[key_aliasName], "sort_g2")).attr("stopEdit","true");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g2")).css("color","transparent");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g4")).attr("stopEdit","true");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g4")).css("color","transparent");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g6")).attr("stopEdit","true");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g6")).css("color","transparent");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g8")).attr("stopEdit","true");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g8")).css("color","transparent");
        ...
        $(getCellObj_idx(p_this[key_aliasName], "sort_g6")).attr("stopEdit","true");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g6")).css("color","transparent");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g10")).attr("stopEdit","true");
        $(getCellObj_idx(p_this[key_aliasName], "sort_g10")).css("color","transparent");
...
function list_json_init() {
    ...
    if($flag=='read') { 
        if($app=='자동정렬' || $grid_load_once_event=='xxxN') {
            $appSql = "select count(num) from mis_companies where sort_g2='0' and useflag=1 ";
            $cnt = onlyOnereturnSql($appSql);
            if($cnt==1) {
                if($MS_MJ_MY=='MY') {
                    $appSql = "call MisCompanyMgt_Ordering_Proc();";
                } else {
                    $appSql = "EXECUTE MisCompanyMgt_Ordering_Proc";
                }
                if(execSql($appSql)) {
                    $resultCode = "success";
...
//exit;
	...
	$AutoGubun = onlyOnereturnSql("select AutoGubun from mis_companies where num='" . $updateList["upidx"] . "'");

	$updateList["sort_g4"] = 0;
	$updateList["sort_g6"] = 0;
	$updateList["sort_g8"] = 0;
	$updateList["sort_g10"] = 0;

	if($AutoGubun=="00") {
		$updateList["AutoGubun"] = "9999";
		$updateList["sort_g2"] = 9999;
	} else if(Len($AutoGubun)==4) {
		$updateList["AutoGubun"] = $AutoGubun . "9999";
		$updateList["sort_g2"] = Left($AutoGubun,4)*1;
		$updateList["sort_g4"] = 9999;
	} else if(Len($AutoGubun)==8) {
		$updateList["AutoGubun"] = $AutoGubun . "9999";
		$updateList["sort_g2"] = Left($AutoGubun,4)*1;
		$updateList["sort_g4"] = Mid($AutoGubun,5,4)*1;
		$updateList["sort_g6"] = 9999;
	} else if(Len($AutoGubun)==12) {
		$updateList["AutoGubun"] = $AutoGubun . "9999";
		$updateList["sort_g2"] = Left($AutoGubun,4)*1;
		$updateList["sort_g4"] = Mid($AutoGubun,5,4)*1;
		$updateList["sort_g6"] = Mid($AutoGubun,9,4)*1;
		$updateList["sort_g8"] = 9999;
	} else if(Len($AutoGubun)==16) {
		$updateList["AutoGubun"] = $AutoGubun . "9999";
		$updateList["sort_g2"] = Left($AutoGubun,4)*1;
		$updateList["sort_g4"] = Mid($AutoGubun,5,4)*1;
		$updateList["sort_g6"] = Mid($AutoGubun,9,4)*1;
		$updateList["sort_g8"] = Mid($AutoGubun,13,4)*1;
		$updateList["sort_g10"] = 9999;
	}
...
function save_writeAfter() {
	...
	if($cnt==1) {
        if($MS_MJ_MY=='MY') {
            $appSql = "call MisCompany_Ordering_Proc();";
        } else {
            $appSql = "EXECUTE MisCompany_Ordering_Proc";
        }
		if(execSql($appSql)) {
			$resultCode = "success";
			$resultMessage = "자동정렬이 완료되었습니다. ";
