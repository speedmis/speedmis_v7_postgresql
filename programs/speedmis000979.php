...
$('body').attr('onlylist','');
  ...
  function 승인(p_idx) {
    ...
    console.log("A 선택");
	url = "add_logic_treat.php?RealPid=<?=$RealPid?>&idx="+p_idx+"&question=confirm&select=Y";
  ...
  function 반려(p_idx) {
	  const dialog = $("#choiceDialog").data("kendoWindow");
    console.log("B 선택");
	  url = "add_logic_treat.php?RealPid=<?=$RealPid?>&idx="+p_idx+"&question=confirm&select=N";
  	temp = ajax_url_return(url);	 
    url = parent.status_view_url();
    url = replaceAll(url, '&tabid=speedmis000979', '');
	  parent.location.replace(url);
...
function add_logic_treat() {
    ...
    if($question=="confirm") {
      if($p_idx!='') {
        $sql = "
        update mis_read_history set 처리결과='$select', readDate=getdate() where idx='$p_idx' and userid='$MisSession_UserID' and 자격 like '결재%';
        ";
      } else if($p_widx!='') {
        $sql = "
        update mis_read_history set 처리결과='$select', readDate=getdate() where widx='$p_widx' and userid='$MisSession_UserID' and 자격 like '결재%';
        ";  
      }
      execSql($sql);
    }
