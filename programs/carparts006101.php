...
function list_json_init() {
    global $real_pid, $mis_join_pid, $logicPid, $parent_idx, $full_siteID;
...
function list_query() {
    ...
    global $real_pid, $mis_join_pid, $logicPid, $parent_idx, $misSessionUserId;
...
//$viewList: 내용전체 array     $saveList: 저장내역 array(aliasName 으로 표현됨)      $updateList: 최종업데이트될 내역(실제필드이름으로 표현됨)
    ...
    //$RealCid = $full_siteID . onlyOnereturnSql("declare @idx int set @idx=ident_current('dbo.mis_common_data')+1 select replicate('0', 6-len(@idx))+convert(nvarchar(6),@idx) ");
...
function addLogic_treat() {
    ...
    if($question=="완료처리") {
		...
		if(Right($status,3)=='금완료' && Right($status_target,3)=='금완료') {
			$sql = "select user_alias from mis_users where user_id='$it_10';";
