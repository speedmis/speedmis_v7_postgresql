...
/*
	...
	if($ActionFlag=='list' && $parent_RealPid=='speedmis000028') {
		...
		$url = "index.php?real_pid=$real_pid&parent_gubun=$target_parent_gubun&parent_idx=$parent_idx";
		re_direct($url);
	}
*/
...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='autogubun') {
		...
		var rValue = '<a href="index.php?real_pid=' + p_dataItem['real_pid'] + '&isMenuIn=Y" target="_blank" class="k-button">Go</a>';
		if(p_dataItem['MenuType']=='01') {
			rValue = rValue + '<a id="aid_' + p_dataItem['idx'] + '" href="index.php?real_pid=speedmis000266&idx=' 
				+ p_dataItem['real_pid'] + '&isMenuIn=Y" target="_blank" class="k-button">Source</a>';
		} else if(p_dataItem['MenuType']=='22') {
			rValue = rValue + '<a id="aid_' + p_dataItem['idx'] + '" href="index.php?real_pid=speedmis000989&idx=' 
				+ p_dataItem['real_pid'] + '&isMenuIn=Y" target="_blank" class="k-button">Source</a>';
		}
...
function save_writeBefore() {
    ...
    global $key_aliasName, $key_value, $ActionFlag, $updateList, $MS_MJ_MY, $base_db;

	if($MS_MJ_MY=='MY') {
	    $RealCid = $full_siteID . onlyOnereturnSql("SELECT formatnums((select AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'mis_common_data' and TABLE_SCHEMA='$base_db'),'000000')");
        $RealCid2 = onlyOnereturnSql("select concat('$full_siteID', formatnums((SELECT MAX(idx) FROM mis_common_data)+1, '000000'));");
		if($RealCid2>$RealCid) {
			$RealCid = $RealCid2;
		}
	} else {
	    $RealCid = $full_siteID . onlyOnereturnSql("declare @idx int set @idx=ident_current('mis_common_data')+1 select dbo.formatnums(@idx,'000000') ");
	}
    $updateList["RealCid"] =  $RealCid;
    $updateList["ggcode"] =  $RealCid;
}
