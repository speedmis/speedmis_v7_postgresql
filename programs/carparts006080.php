...
JOIN v_parts_cate_tree_a AS vc
...
UPDATE car_parts AS cp
JOIN vv_parts_storage_tree AS vs
...
function addLogic_treat() {
	...
	if($question=="move") {
		$sql = " select sort_g01,sort_g02,sort_g03,depth from parts_cate_a where idx='$p_idx' ";
		$r = allreturnSql($sql);
		$depth = $r[0]['depth'];
		$sort_g01 = $r[0]['sort_g01'];
		$sort_g02 = $r[0]['sort_g02'];
		$sort_g03 = $r[0]['sort_g03'];
		if($depth==1) {
			if($p_move=='top') {
				$sort_g01 = '0.5';
			} else if($p_move=='up') {
				$sort_g01 = $sort_g01 - 1.5;
			} else if($p_move=='down') {
				$sort_g01 = $sort_g01 + 1.5;
		...
		$sql = " 
		update parts_cate_a set sort_g01='$sort_g01',sort_g02='$sort_g02',sort_g03='$sort_g03'
		where idx='$p_idx';
		call parts_cate_a_ordering_proc();
		";
		//echo $sql;
		execSql($sql);
		echo 'success';
		...
		$sql = " select sort_g01,sort_g02,sort_g03
		,upidx,depth from parts_cate_a where idx='$p_idx' ";
		$r = allreturnSql($sql);
		...
		$sql = " 
		insert into parts_cate_a (cate_name, upidx, sort_g01, sort_g02, sort_g03
		, depth, wdater) 
		values (N'$add_name', $upidx, '$sort_g01', '$sort_g02', '$sort_g03'
