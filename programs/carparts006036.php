...
INSERT INTO mis_attach_list (table_name,field_name,idx_name,idx_num,attach_url,attach_size,attach_mime,attach_name, wdate, lastupdate)
SELECT 'car_parts' AS table_name, 'photos' AS field_name, 'id' AS idx_name, fileable_id AS idx_num
,real_path AS attach_url, file_size as attach_size, mime AS attach_mime, file_name AS attach_name, created_at, updated_at
from files WHERE real_path NOT IN 
(SELECT attach_url FROM mis_attach_list WHERE table_name='car_parts')
ORDER BY id;
-- 2-2) mis_attach_list 의 midx  업데이트하기
UPDATE mis_attach_list AS main
JOIN (
    -- 1. 각 idx_num 그룹별로 최대 idx 값을 찾는 서브쿼리
    SELECT
        idx_num,
...
JOIN mis_attach_list AS mal
    -- 연결 조건: car_parts.id = mis_attach_list.idx_num
    -- mis_attach_list의 table_name 값이 'car_parts'인 경우만 조인 대상에 포함
ON 
    cp.id = mal.idx_num AND mal.table_name = 'car_parts'
SET
    -- 업데이트 대상: car_parts.photos_midx에 mis_attach_list.midx 값을 설정
...
UPDATE car_parts AS cp
JOIN (
    -- 1. midx별로 attach_name을 '@AND@'로 연결하는 서브쿼리
    SELECT
        midx,
        GROUP_CONCAT(attach_name ORDER BY attach_name ASC SEPARATOR '@AND@') AS photos_list
    FROM
        mis_attach_list
    GROUP BY
        midx
...
ON
    -- 연결 조건: car_parts의 photos_midx와 mis_attach_list의 midx 일치
...
-- ---------------------------------------------------------------------
-- mis_attach_list와 car_parts 간의 midx 불일치 및 중복 오류를 수정하는 쿼리입니다.
-- (mis_attach_list에 대해 midx가 2개 이상 사용된 경우, 큰 값으로 통일합니다.)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2. [car_parts 업데이트 - 1순위] mis_attach_list의 오류 그룹을 찾아
--    car_parts.photos_midx를 최대 midx 값으로 먼저 업데이트
-- ---------------------------------------------------------------------

UPDATE car_parts AS cp
JOIN temp_midx_fix AS tf
    -- 조인 조건: car_parts.id = 오류그룹.idx_num
ON 
    cp.id = tf.idx_num
SET
...
-- ---------------------------------------------------------------------
-- 3. [mis_attach_list 업데이트 - 2순위] mis_attach_list의 midx 오류를 최대 midx 값으로 통일
-- ---------------------------------------------------------------------

UPDATE mis_attach_list AS mal_main
JOIN temp_midx_fix AS tf
ON
    -- 조인 조건: idx_num은 일치하지만, midx 값이 최대 midx 값과 다른 레코드
    mal_main.idx_num = tf.idx_num AND mal_main.midx != tf.correct_midx
SET
    -- 업데이트: midx를 그룹의 최대 midx 값으로 통일
    mal_main.midx = tf.correct_midx;

-- ---------------------------------------------------------------------
-- 4. [photos 업데이트 - 최종] photos_midx 값을 기반으로 attach_name을 연결하여 car_parts.photos 필드를 업데이트
-- ---------------------------------------------------------------------
UPDATE car_parts AS cp
JOIN (
    -- 1. midx별로 attach_name을 '@AND@'로 연결하는 서브쿼리
...
JOIN temp_midx_fix AS tf ON cp.id = tf.idx_num
SET
...
SELECT 'App\\Models\\CarPart' AS fileable_type, idx_num AS fileable_id
, attach_name AS file_name, REPLACE(attach_url,'/storage/','') AS saved_name
, attach_url AS real_path, attach_size AS file_size
, case when RIGHT(attach_name,4)='.jpg' OR RIGHT(attach_name,4)='jpeg' then 'image/jpeg'
when RIGHT(attach_name,4)='.png' then 'image/png' ELSE 'application/zip' END AS mime
,'public' AS STORAGE
, case when RIGHT(attach_name,4)='.jpg' OR RIGHT(attach_name,4)='jpeg' then 'image'
when RIGHT(attach_name,4)='.png' then 'image' ELSE 'zip' END AS type
, wdate AS created_at, lastupdate AS updated_at
 from mis_attach_list WHERE table_name='car_parts' AND idx_num=$key_value
 ORDER BY idx; 
";
...
if($virtual_fieldQngrade_a!=0 || $virtual_fieldQngrade_b!=0 || $virtual_fieldQngrade_c!=0) {
	...
	INSERT INTO parts_add_log (car_parts_id, grade_a_add, grade_b_add, grade_c_add, grade_a, grade_b, grade_c, remark)
...
function save_writeQueryBefore() {
	...
	global $full_siteID, $base_root, $real_pid, $mis_join_pid, $logicPid, $parent_idx, $misSessionUserId;
...
when RIGHT(attach_name,4)='.png' then 'image' ELSE 'zip' END AS type
, wdate AS created_at, lastupdate AS updated_at
 from mis_attach_list WHERE table_name='car_parts' AND idx_num=$newIdx
 ORDER BY idx; 
";
//echo $sql;exit;
	execSql($sql);
...
function addLogic_treat() {
    ...
    if($top_img!='') {
		...
		$sql = " 
SELECT idx FROM  mis_attach_list WHERE table_name='car_parts' and idx_num=$idx
...
";
		...
		$sql = " 
SELECT min(idx) FROM  mis_attach_list WHERE table_name='car_parts' and idx_num=$idx
";
