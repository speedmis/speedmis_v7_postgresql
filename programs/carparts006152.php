<?php
/**
 * carparts006152.php — 쿠팡 주문 동기화 현황
 *
 * 화면 동작:
 *  - 기본 리스트 — mis_coupang_orders 표시 (주문시각 내림차순)
 *  - 상품명 컬럼 클릭 → 부자톡 상품 (carparts006083 / gubun=6083) 새 탭
 *  - 헤더 [지금 동기화] 버튼 → coupang_order_sync.php 즉시 실행 → 토스트
 *
...
/**
 * 커스텀 버튼 [지금 동기화] 핸들러 — coupang_order_sync.php 즉시 호출.
...
function list_json_init() {
	...
	$url    = $base . '/_coupang/coupang_order_sync.php';
	...
	if ($http === 200 && !empty($res['success'])) {
		$msg = sprintf(
			'쿠팡 동기화 완료 — 수집%d / 신규%d / 차감%d / 환원%d (%dms)',
			$res['fetched']        ?? 0,
			$res['inserted']       ?? 0,
		...
		$GLOBALS['_client_alert'] = "쿠팡 동기화 호출 실패 (HTTP {$http})\n" . substr($body, 0, 500);
	}
}
