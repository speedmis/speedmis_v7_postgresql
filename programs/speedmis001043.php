...
function update_RealPid(p_this, p_RealPid, p_updateVersion) {
	...
	if(location.host=='www.speedmis.com') {
		url = "/_mis/addLogic_treat.php?real_pid="+$('#real_pid').val()+"&targetID="+p_RealPid;
		p_updateVersion = ajax_url_return2(url);
	}
	update_program_url = "misShare_models_save.php?real_pid="+p_RealPid+"&mis_join_pid=&key_aliasName=real_pid&updateVersion=<?php echo iif($MS_MJ_MY=='MY','MY_',''); ?>"+p_updateVersion;
...
function columns_templete(p_dataItem, p_aliasName) {
    if(p_aliasName=="zrokeolbeojeommicheopdeiteu") {
        localVersion = ajax_url_return2("/_mis_addLogic"+"/updateVersion/updateVersion_"+p_dataItem["real_pid"]+".txt");
		...
		if(p_dataItem["updateVersion"] != localVersion) {
			if('<?php echo $full_siteID; ?>'=='speedmy' && p_dataItem["updateVersion"] < localVersion) {
				rValue = localVersion + '<a onclick="update_RealPid(this, \''+p_dataItem["real_pid"]+'\',\''+localVersion+'\');" style="margin-left:10px;" class="k-button">업데이트</a>';
			} else {
				rValue = localVersion + '<a onclick="update_RealPid(this, \''+p_dataItem["real_pid"]+'\',\''+p_dataItem["updateVersion"]+'\');" style="margin-left:10px;" class="k-button">업데이트</a>';
			}
		} else {
			rValue = localVersion + '<a onclick="update_RealPid(this, \''+p_dataItem["real_pid"]+'\',\''+p_dataItem["updateVersion"]+'\');" style="margin-left:10px;" class="k-button">완료됨</a>';
		}
		rValue = rValue + '<a target="_blank" href="index.php?real_pid='+p_dataItem['real_pid']+iif(getID('isMenuIn').value=='Y','&isMenuIn=Y','')+'" style="margin-left:10px;" class="k-button">연결</a>';
        return rValue;
    } else {
...
function rowFunction_UserDefine(p_this) {
    ...
    //p_this.autogubun = 
    //"<a href=index.asp?gubun=" + p_this.idx + "&isMenuIn=Y target=_blank>[Go]</a> <a id='aid_" + p_this.idx + "' href=index.asp?real_pid=speedmis000266&idx=" + p_this.idx + "&isMenuIn=Y target=_blank>[Source]</a>?" 
    //+ p_this.autogubun; 
}
...
function thisLogic_toolbar() {
	...
	if(autoUpdate=='Y') {
		...
		$("#btn_2").click( function() {
			url = "addLogic_treat.php?real_pid="+$('#real_pid').val()+"&autoUpdate=N";
			ajax_url_touch(url);
			setTimeout( function() {
		...
		$("#btn_2").css("color", "#fff");
		$("#btn_2").click( function() {
			url = "addLogic_treat.php?real_pid="+$('#real_pid').val()+"&autoUpdate=Y";
			ajax_url_touch(url);
			setTimeout( function() {
				location.href = location.href;
			},500);
...
function list_json_init() {
    global $real_pid, $mis_join_pid, $logicPid, $parent_idx, $full_site, $full_siteID, $MS_MJ_MY, $addDir;
...
function addLogic_treat() {
	...
	if($autoUpdate!='') {
		...
		$sql = " update mis_shares set updateVersion='$new_updateVersion' where real_pid='$targetID' ";
		execSql($sql);
