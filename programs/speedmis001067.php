...
function columns_templete(p_dataItem, p_aliasName) {
    if(p_aliasName=="real_pid") {
		var rValue = "<a href='index.php?real_pid=" + p_dataItem["real_pid"] + "&isMenuIn=Y' target='_blank' class='k-button'>열기</a>";

		rValue = p_dataItem["real_pid"] + " " + rValue;
        return rValue;
    } else {
...
function rowFunctionAfter_UserDefine(p_this) {
    ...
    "<a href=index.php?real_pid=" + p_this.real_pid + "&isMenuIn=Y target=_blank>[Go]</a>"+iif(p_this.MenuType=="01", "<a id='aid_" + p_this.idx + "' href=index.php?real_pid=speedmis000266&idx=" 
    + p_this.real_pid + "&isMenuIn=Y target=_blank>[Source]</a>","")
    + "?" + p_this.autogubun; 
    */
...
function list_json_init() {
    global $real_pid, $mis_join_pid, $logicPid, $parent_idx, $full_siteID, $MS_MJ_MY;
