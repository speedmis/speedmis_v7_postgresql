...
function delStyleIdx(p_this) {
	
	url = "addLogic_treat.php?RealPid=<?=$RealPid?>&stationIdx="+$(p_this).attr('stationIdx')+"&styleIdx="+$(p_this).attr('styleIdx')+"&question=delStyleIdx";
...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='styQnstyleName') {
	
		rValue = p_dataItem[p_aliasName];
		if(rValue!='' && rValue!=undefined) {
			var rValue = '<a href="javascript:;" onclick="delStyleIdx(this);" stationIdx="'+p_dataItem['idx']+'" styleIdx="'+p_dataItem['docQnstyleIdx']+'" class="k-button">연결끊기</a>';
			rValue = rValue + p_dataItem[p_aliasName];
		}
...
function list_json_updateBefore() {

    global $base_root, $RealPid, $MisJoinPid, $logicPid, $parent_idx, $up_keyValue, $MisSession_UserID;
    global $flag, $app, $idx, $appSql, $resultCode, $resultMessage, $afterScript,$execSql;
    global $data, $key_aliasName, $parent_alias, $sql_update, $up_field, $up_fieldValue, $request_body;
	//gzecho(json_encode($request_body));exit;
	//gzecho($up_field . ';;;'. $up_fieldValue);exit;


	//echo "stationIdx=$up_keyValue; styleIdx=$up_fieldValue;";
	if($up_field=='virtual_fieldQnaddDocs' && $up_keyValue!='' && $up_fieldValue!='') {
		$execSql = " if not exists(select * from kimgoDocsList where stationIdx=$up_keyValue and styleIdx=$up_fieldValue) begin
			insert into kimgoDocsList (stationIdx,styleIdx,wdater) values ($up_keyValue,$up_fieldValue,N'$MisSession_UserID');
		end";
		execSql($execSql);
...
function addLogic_treat() {
    ...
    if($question=="delStyleIdx") {
		$stationIdx = requestVB("stationIdx");
		$styleIdx = requestVB("styleIdx");
		$sql = " delete kimgoDocsList where stationIdx=$stationIdx and styleIdx=$styleIdx";
		execSql($sql);
