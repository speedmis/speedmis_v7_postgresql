...
function misMenuList_change() {
	...
	//기본필터의 예
	if($RealPid=='carparts006125') {
		$result[0]['g09'] = "and not (si.it_use=1 and si.ca_id<>'' and si.ca_id<>'4007') and table_m.site_id='00.부자톡' and table_m.steps in ('상신','검수완료','비교불가')";
	}
...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='zbullyu') {
		...
		rValue = rValue + '<a href="https://xn--or3b27p5mi.com/shop/item.php?it_id=' 
...
function viewLogic_afterLoad() {
    ...
    //기본폼 우측탭 중에 리스트형 프로그램과 2단으로 조회하려면 아래 주석을 풀고 번호를 넣을 것.
    form1_and_list_tabnumer(2); 
...
FROM (
    SELECT '$idx' AS it_id, '00.부자톡' AS site_id UNION ALL
    SELECT '$idx', '01.이베이' UNION ALL
    SELECT '$idx', '02.번개장터' UNION ALL
    SELECT '$idx', '03.ok파츠' UNION ALL
    SELECT '$idx', '04.GK파츠' UNION ALL
    SELECT '$idx', '05.파츠핏' UNION ALL
    SELECT '$idx', '06.G파츠' UNION ALL
    SELECT '$idx', '07.쿠팡' UNION ALL
    SELECT '$idx', '08.네이버스토어' UNION ALL
    SELECT '$idx', '09.중고나라' UNION ALL
    SELECT '$idx', '10.구글' UNION ALL
    SELECT '$idx', '11.기타원' UNION ALL
    SELECT '$idx', '12.기타원' UNION ALL
    SELECT '$idx', '13.기타$'
) AS tmp
WHERE NOT EXISTS (
    SELECT 1 
