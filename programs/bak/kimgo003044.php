...
div#grid div.k-grid-content.k-auto-scrollable {
	display: contents;
...
function listLogic_afterLoad_once()	{
	...
	//$('div#grid div.k-grid-content.k-auto-scrollable')[0].style.setProperty('height', 'auto', 'important');
}
...
function list_json_init() {
    global $RealPid, $MisJoinPid, $logicPid, $parent_idx, $full_siteID, $grid_load_once_event, $MisSession_UserID;
    ...
    if($grid_load_once_event=='N') { 
            $appSql = "select count(*) from kimgoStyle_Detail where midx=$parent_idx";
            $cnt = onlyOnereturnSql($appSql);
            if($cnt==0) {
                $appSql = '';
                for($ii=1;$ii<21;$ii++) {
$d_field = ''; $d_value = '';
$d_midx = $parent_idx; $d_field = $d_field . 'midx'; $d_value = $d_value . "'$d_midx'";
$d_SortElement = $ii; $d_field = $d_field . ',SortElement'; $d_value = $d_value . ",'$d_SortElement'";
$d_Grid_Columns_Title = "타이틀 $d_SortElement"; $d_field = $d_field . ',Grid_Columns_Title'; $d_value = $d_value . ",'$d_Grid_Columns_Title'";
$d_Grid_Columns_Width = '10'; $d_field = $d_field . ',Grid_Columns_Width'; $d_value = $d_value . ",'$d_Grid_Columns_Width'";
$d_Grid_CtlName = 'text'; $d_field = $d_field . ',Grid_CtlName'; $d_value = $d_value . ",'$d_Grid_CtlName'";
$d_Grid_MaxLength = '50'; $d_field = $d_field . ',Grid_MaxLength'; $d_value = $d_value . ",'$d_Grid_MaxLength'";
$d_wdater = $MisSession_UserID; $d_field = $d_field . ',wdater'; $d_value = $d_value . ",'$d_wdater'";
$appSql = $appSql . "insert into kimgoStyle_Detail ($d_field) values 
($d_value);
";
                }
$appSql = $appSql . "
update kimgoStyle_Detail set remark='긴문자전용', Grid_CtlName='textarea', Grid_MaxLength=4000 where midx=$parent_idx and SortElement between 13 and 14;
update kimgoStyle_Detail set remark='숫자전용', Grid_Schema_Type='number^^#,##0 원', Grid_MaxLength=4000 where midx=$parent_idx and SortElement between 15 and 19;
update kimgoStyle_Detail set remark='첨부파일전용', Grid_CtlName='attach', Grid_MaxLength=10 where midx=$parent_idx and SortElement=20;
";

                if(execSql($appSql)) {
                    $resultCode = "success";
                    $resultMessage = "기본내역이 추가되었습니다.";
                } else {
                    $resultCode = "fail";
                    $resultMessage = "기본내역이 추가가 실패하였습니다.";
                }
            }
			...
			select  idx,SortElement from kimgoStyle_Detail where midx=$parent_idx order by SortElement, idx 
			...
			BEGIN    
			set @ii = @ii+1
			set @sql = @sql + ' update kimgoStyle_Detail set SortElement='+convert(nvarchar(5),@ii)+' where idx='+convert(nvarchar(5),@idx)
