...
function misMenuList_change() {
    ...
    global $ActionFlag, $gubun, $parent_idx, $RealPid, $logicPid, $result, $MisSession_UserID, $MisSession_StationName;
	global $MisSession_PositionCode, $flag, $list_check_hidden, $orderby;

	if($RealPid=='carparts006086' || $RealPid=='carparts006096') {
		$result[0]['g09'] = "and table_m.it_use=1 and table_m.ca_id<>'' and table_m.ca_id<>'4007' and table_m.it_stock_qty>0";
	} else if($RealPid=='carparts006084') {
		$result[0]['g09'] = "and not (table_m.it_use=1 and table_m.ca_id<>'' and table_m.ca_id<>'4007') and table_m.part_union_level<>3";
		
	} else if($RealPid=='carparts006094') {
		$result[0]['g09'] = "and not (table_m.it_use=1 and table_m.ca_id<>'' and table_m.ca_id<>'4007') ";
		
	} else if($RealPid=='carparts006123') {
		$result[0]['g09'] = "and not (table_m.it_use=1 and table_m.ca_id<>'' and table_m.ca_id<>'4007')  and ic.select_site_id is null and table_m.part_union_level<>3";
	} else if($RealPid=='carparts006124') {
		$result[0]['g09'] = "and not (table_m.it_use=1 and table_m.ca_id<>'' and table_m.ca_id<>'4007')  and ifnull(ic.select_site_id,'')<>''";
	} else if($RealPid=='carparts006085' || $RealPid=='carparts006095') {
		$result[0]['g09'] = "and table_m.it_use=1 and table_m.ca_id<>'' and table_m.ca_id<>'4007' and table_m.it_stock_qty=0";
	} else if($RealPid=='carparts006129') {
		$result[0]['g09'] = "and not (table_m.it_use=1 and table_m.ca_id<>'' and table_m.ca_id<>'4007') and table_m.part_union_level=3";
	}

	if($RealPid=='carparts006074' || $RealPid=='carparts006094' || $RealPid=='carparts006095' || $RealPid=='carparts006096') {
	...
	} else if($RealPid=='carparts006111' || $RealPid=='carparts006112') {		//이베이
/*
...
function pageLoad() {
	...
	global $MisSession_UserID, $MisSession_IsAdmin, $parent_RealPid, $MisSession_StationName;
...
body[userid="<?php echo $MisSession_UserID; ?>"][actionflag="modify"] .before_round_virtual_fieldQnprint_request
,body[userid="<?php echo $MisSession_UserID; ?>"][actionflag="modify"] div#round_virtual_fieldQnprint_request
,body[userid="<?php echo $MisSession_UserID; ?>"][actionflag="modify"] div#round_print_request_time
,body[userid="<?php echo $MisSession_UserID; ?>"][actionflag="modify"] div#round_print_response_time
{
...
function list_json_init() {
    global $allFilter, $RealPid, $MisJoinPid, $logicPid, $parent_idx, $full_siteID, $grid_load_once_event, $MisSession_UserID;
	...
	global $temp3;
	if($flag=='read' && $RealPid=='carparts006121') {
		...
		if(InStr($app, '@@END_QUERY;')>0) {
			...
			$sql = "
insert into g5_shop_item_update_log (msg, uquery, wdater) values ('$msg','$appSql','$MisSession_UserID');
			";
			execSql($sql);
			$resultCode = 'success';
...
values ('$idx', '$it_10_old', '$it_10_new', '$MisSession_UserID');
