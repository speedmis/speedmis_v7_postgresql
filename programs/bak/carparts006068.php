...
function list_json_load() {
    ...
    global $base_root, $RealPid, $MisJoinPid, $logicPid, $parent_idx,$addParam, $MisSession_UserID, $ServerVariables_REMOTE_ADDR;
    ...
    if($flag=='readResult' && InStr($addParam,'print')>0) { 
        ...
        if($count>0) {
			...
			if($flag=='창고라벨인쇄') {
				...
				//globalautoparts.co.kr 로 안하면 큐알 인식이 안됨.
...
";
				...
				if(InStr($addParam,'print.nocount')==0) {
				...
				$pn = $p_data[0]['part_number'];
				$pn = uni_left($pn,26);
				$full_name_a = $p_data[0]['full_name_a'];
				...
				$full_name_b = $p_data[0]['full_name_b'];
...
";
//gzecho($ServerVariables_REMOTE_ADDR);
				if(InStr($addParam,'print.nocount')==0) {
				...
				//if($MisSession_UserID=='gadmin' || $ServerVariables_REMOTE_ADDR=='175.113.113.120') {
					gzecho($qr_format);
				//}
			}
