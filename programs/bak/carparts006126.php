...
function pageLoad() {
    ...
    global $ActionFlag, $RealPid, $parent_idx, $idx, $logicPid;
...
if(location.href.split('parent_idx').length==2) {
	if(parent.result['assy_yn']=='Y') {
		url = `index.php?gubun=`+document.getElementById('gubun').value+`&allFilter=[{"operator":"contains","value":"=`+parent.result['parts_union']+`","field":"parts_union"}]&orderby=assy_define_sort_num&recently=N&isAddURL=Y`;
		location.href = url;
	}
}
...
function order_treat(p_it_id) {
	
	url = "addLogic_treat.php?RealPid=<?=$logicPid?>&treat_it_id="+p_it_id+"&question=assy_in_end_treat";
	temp = ajax_url_return(url).split(";");
	if(InStr(temp,'정상적으로')>0) {
...
function columns_templete(p_dataItem, p_aliasName) {
    if(p_aliasName=='it_id') {
		url = "index.php?gubun=6116&idx="+p_dataItem['ca_car_id']+"_-_"+p_dataItem['ca_id']+"&ActionFlag=modify";
		var rValue = p_dataItem[p_aliasName]+'<br><a onclick="window.open(`'+url+'`);" class="k-button">Assy정의창</a>';
		 
        return rValue;
    } else if(p_aliasName=='zassysumbeonsajin' || p_aliasName=='zhaedangbupumsajin') {
		var rValue = p_dataItem[p_aliasName];
		if(rValue!='' && rValue!=null) {
			rValue = '<img src="thumbnail.php?'+rValue+'" style="max-height:100px;"/>';
		} 
        return rValue;
    } else if(p_aliasName=='virtual_fieldQnin_order_qty') {
		var rValue = p_dataItem['oaQnin_order_qty'];
		 
        return rValue;
    } else if(p_aliasName=='virtual_fieldQnin_order_price') {
		var rValue = p_dataItem['oaQnin_order_price'];
		 
        return rValue;
    } else if(p_aliasName=='virtual_fieldQnin_order_treat') {
		var rValue = '<a onclick="order_treat('+p_dataItem['it_id']+');" class="k-button">재고에 반영</a>';
...
function rowFunction_UserDefine(p_this) {
	...
	p_this.virtual_fieldQnin_order_qty = p_this.oaQnin_order_qty; 
	p_this.virtual_fieldQnin_order_price = p_this.oaQnin_order_price; 
...
function list_json_init() {
    global $RealPid, $MisJoinPid, $logicPid, $parent_idx, $full_siteID, $grid_load_once_event;
    ...
    if($flag=='read' && $grid_load_once_event=='N') { 
            $appSql = "
-- assy 부족영역 자동입력
INSERT INTO g5_shop_item (ca_car_id, ca_id, assy_define_idx, it_name, part_union_level)
select distinct table_m.car_code as `car_code`
,table_m.cate_code as `cate_code`
,table_m.idx as `assy_define_idx`
,CONCAT('assy 부족영역 자동입력:',table_m.sort_num,'번') AS it_name
,3 as part_union_level
from mis_parts_assy_define_detail table_m
left outer join g5_shop_item si ON si.parts_union=concat(table_m.car_code,'.', table_m.cate_code)
where table_m.useflag='1'

AND concat(si.parts_union,'.',table_m.idx) NOT IN (

SELECT concat(parts_union,'.',assy_define_idx) from v_g5_shop_item_assy_stocks
);

UPDATE mis_parts_assy_define_detail ad
SET ad.it_id = (
    SELECT si.it_id
    FROM g5_shop_item si
    WHERE si.assy_define_idx = ad.idx
      AND si.part_union_level = 3
    LIMIT 1
)
WHERE ad.it_id IS NULL;
...
function textUpdate_sql() {
    ...
    global $strsql, $keyAlias, $keyValue, $thisValue, $oldText, $thisAlias, $resultCode, $resultMessage, $afterScript,$MisSession_UserID;

	//아래는 특정항목을 수정할 경우, 해당항목이 정의된 리스트에 포함되었을 경우, 관련 업데이트문을 추가하고, 처리메세지를 브라우저로 전달하는 로직입니다.  
    if($thisAlias=='virtual_fieldQnin_order_qty') {
        $addSql = " 
INSERT INTO mis_parts_assy_item_orders_active (it_id, in_order_qty, wdater)
VALUES ('$keyValue', $thisValue, '$MisSession_UserID')
ON DUPLICATE KEY UPDATE
    in_order_qty = VALUES(in_order_qty);
		";
		execSql($addSql);
	} else if($thisAlias=='virtual_fieldQnin_order_price') {
        $addSql = " 
INSERT INTO mis_parts_assy_item_orders_active (it_id, in_order_price, wdater)
VALUES ('$keyValue', $thisValue, '$MisSession_UserID')
ON DUPLICATE KEY UPDATE
    in_order_price = VALUES(in_order_price);
		";
		execSql($addSql);
	}
//echo $addSql;exit;
...
function addLogic_treat() {
    ...
    $treat_it_id = requestVB("treat_it_id");

	//아래는 값에 따라 mysql 서버를 통해 알맞는 값을 출력하여 보냅니다.
    if($question=="assy_in_end_treat") {

	
		$sql = " select * from mis_parts_assy_item_orders_active where it_id='$treat_it_id' ";
		$r = allReturnSql($sql);
		if(count($r)==0) {
echo '해당 품목이 존재하지 않습니다.';
exit;
		}
		$in_order_qty = $r[0]['in_order_qty']*1;
		$in_order_price = $r[0]['in_order_price']*1;
		if($in_order_qty<=0) {
echo '정확한 수량을 입력하세요';
exit;
		}
...
exit;
        ...
        $sql = " 
insert into mis_parts_assy_item_orders_all (it_id, in_order_qty, in_order_price, wdater) values ('$treat_it_id', $in_order_qty, $in_order_price, '$MisSession_UserID');
update g5_shop_item set it_stock_qty=it_stock_qty+$in_order_qty,it_price_cost=$in_order_price where it_id='$treat_it_id';
update mis_parts_assy_item_orders_active set in_order_qty=0, in_order_price=0 where it_id='$treat_it_id';
		";

		execSql($sql);
		
		echo '정상적으로 입고처리되었습니다.';
