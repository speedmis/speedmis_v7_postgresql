...
function list_json_init() {
    ...
    if($flag=='read') {
        if($app=='DB반영') {
            $appSql = "select count(idx) from parts_cate_b where sort_g01='0' and useflag='1' ";
            $cnt = onlyOnereturnSql($appSql);
            if($cnt==1) {
                if($MS_MJ_MY=='MY') {
                    $appSql = "call parts_cate_b_ordering_proc();";
                } else {
                    $appSql = "EXECUTE parts_cate_b_ordering_proc";
                }
                if(execSql($appSql)) {
                    $resultCode = "success";
...
//exit;
	...
	$autogubun = onlyOnereturnSql("select autogubun from parts_cate_b where idx='" . $updateList["upidx"] . "'");
