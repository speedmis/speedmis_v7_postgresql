...
<div id="dialog"></div>
        ...
        if($MS_MJ_MY=='MY') {
            ...
            ,case when ifnull(table_m.title,'')<>'' then table_m.title else table_RealPid.menu_name end as menu_name
            ,table_m.RealPid as 'RealPid'
            ,table_RealPid.MisJoinPid
            ,table_RealPid.idx as 'gubun'
            ,table_RealPid.menu_type
            ...
            from mis_favorite_menus table_m   
            left outer join mis_menus table_RealPid on table_RealPid.RealPid = table_m.RealPid and (table_RealPid.menu_type in ('01','02','04','06','21','22') 
            and table_RealPid.useflag=1 and table_RealPid.menu_name not like 'x%')   
            left outer join mis_menu_auth table_member on table_member.RealPid = table_RealPid.RealPid and table_member.userid='" . $MisSession_UserID . "'   
            ...
            $sql = "select table_m.idx
            ,case when isnull(table_m.title,'')<>'' then table_m.title else table_RealPid.menu_name end as menu_name
            ,table_m.RealPid as 'RealPid'
            ,table_RealPid.MisJoinPid
            ,table_RealPid.idx as 'gubun'
            ,table_RealPid.menu_type
            ...
            while ($mm<$cnt_result) {
                //print_r($mm . ":" . $result[$mm]["menu_name"]);
                $dash_gubun = $result[$mm]["gubun"];
                $dash_RealPid = $result[$mm]["RealPid"];
                $dash_misJoinPid = $result[$mm]["MisJoinPid"];
                $dash_menuname = $result[$mm]["menu_name"];
                $dash_MenuType = $result[$mm]["menu_type"];
        ...
        <?php
                    ...
                    if($MS_MJ_MY=='MY') {
                        $sql = "select table_m.idx
                    ,table_m.alias_name as 'field'
                    ,table_m.col_title as 'title'
                    ,case when table_m.col_width=0 or table_m.col_width=-1 then 1 else 0 end as 'hidden'
                    ,14+8*abs(table_m.col_width) as 'width'
                    ,table_m.col_width as 'width0'
                    ,table_m.Grid_Align
                    ,table_m.Grid_Orderby
                    ,table_m.schema_type
                    from mis_menu_fields table_m   
                    where table_m.useflag='1' and table_m.alias_name<>'' and table_m.RealPid = '" . iif($dash_MenuType=="06", $dash_misJoinPid, $dash_RealPid) . "'
                    and (table_m.sort_order<=15 or (table_m.col_width<-1 or table_m.col_width>0) or isnumeric(left(table_m.Grid_Orderby,1))=1)
                    order by case when table_m.sort_order<=2 then 0 else 1 end, table_m.sort_order limit 40;";
                    } else {
                    ...
                    where table_m.useflag='1' and table_m.alias_name<>'' and table_m.RealPid = '" . iif($dash_MenuType=="06", $dash_misJoinPid, $dash_RealPid) . "'
                    and (table_m.sort_order<=15 or (table_m.col_width<-1 or table_m.col_width>0) or isnumeric(left(table_m.Grid_Orderby,1))=1)
                    order by case when table_m.sort_order<=2 then 0 else 1 end, table_m.sort_order";
                    }
    ...
    $("#btn_1").click( function() {
        ...
        return false;
            ...
            $(document).ready(function() {
				...
				if($only_gadmin_YN=="Y") $sql = "select mainSortable from mis_users where uniquenum=N'gadmin';";
				else $sql = "select mainSortable from mis_users where uniquenum=N'" . $MisSession_UserID . "';";
    ...
    function viewModel_bind() {
        ...
        <?php
            ...
            while ($mm<$cnt_result) {
                ...
                $dash_AddURL = $result[$mm]["AddURL"];
                $dash_AddURL = replace($dash_AddURL, '&orderby=', '&$orderby=');
                $dash_AddURL = replace(replace($dash_AddURL, "@misSessionUserId", $MisSession_UserID), "{MisSession_UserID}", $MisSession_UserID);
                ...
                if(Left($dash_MenuType,1)=="0") {
                    ...
                    from mis_menu_fields table_m   
                    where table_m.useflag='1' and table_m.alias_name<>'' and table_m.RealPid = '" . iif($dash_MenuType=="06", $dash_misJoinPid, $dash_RealPid) . "'
                    and (table_m.sort_order<=8 or (table_m.col_width<-1 or table_m.col_width>0) or isnumeric(left(table_m.Grid_Orderby,1))=1)
                    order by case when table_m.sort_order<=2 then 0 else 1 end, table_m.sort_order";
                    ...
                    while ($mm2<$cnt_result2) {
                        ...
                        $data_SortElement = $result2[$mm2]["sort_order"];
                        $data_aliasName = $result2[$mm2]["alias_name"];
                        $data_Grid_CtlName = $result2[$mm2]["Grid_CtlName"];
                        $data_Grid_Schema_Type = $result2[$mm2]["schema_type"];
