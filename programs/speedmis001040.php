...
function list_json_load() {
    ...
    global $data, $key_aliasName, $child_alias, $selectQuery, $keyword, $menu_name, $_count;
	global $allFilter, $speed_Grid_Columns_Title,$full_site, $MisSession_UserID;

	global $commentSql, $commentData, $full_siteID;

	if($full_siteID=='speedmis' && $flag=='read' && $selField=='' && $commentSql!='') {
		$commentSql2 = replace($commentSql, 'mis_comments', 'Mis_' . '_speedsup_' . '_Comments');
