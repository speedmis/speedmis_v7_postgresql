...
function pageLoad() {
    ...
    global $actionFlag, $real_pid, $parent_idx, $idx, $logicPid;
	global $misSessionUserId, $misSessionIsAdmin, $parent_real_pid;
/*
	//특정상황에서 페이지를 이동시키는 예제입니다.
	if($actionFlag=='list' && $parent_real_pid=='speedmis000028') {
...
function list_json_init() {
    global $real_pid, $mis_join_pid, $logicPid, $parent_idx, $full_siteID, $grid_load_once_event;
...
from parts_assy_define_detail table_m
...
UPDATE parts_assy_define_detail ad
SET ad.it_id = (
    SELECT si.it_id
...
function textUpdate_sql() {
    ...
    if($thisAlias=='virtual_fieldQnin_order_qty') {
        $addSql = " 
INSERT INTO parts_assy_item_orders_active (it_id, in_order_qty, wdater)
VALUES ('$keyValue', $thisValue, '$misSessionUserId')
ON DUPLICATE KEY UPDATE
    in_order_qty = VALUES(in_order_qty);
        ...
        $addSql = " 
INSERT INTO parts_assy_item_orders_active (it_id, in_order_price, wdater)
VALUES ('$keyValue', $thisValue, '$misSessionUserId')
ON DUPLICATE KEY UPDATE
...
function addLogic_treat() {
    ...
    if($question=="assy_in_end_treat") {
		...
		$sql = " select * from parts_assy_item_orders_active where it_id='$treat_it_id' ";
		$r = allReturnSql($sql);
		if(count($r)==0) {
...
exit;
        ...
        $sql = " 
insert into parts_assy_item_orders_all (it_id, in_order_qty, in_order_price, wdater) values ('$treat_it_id', $in_order_qty, $in_order_price, '$misSessionUserId');
update g5_shop_item set it_stock_qty=it_stock_qty+$in_order_qty,it_price_cost=$in_order_price where it_id='$treat_it_id';
update parts_assy_item_orders_active set in_order_qty=0, in_order_price=0 where it_id='$treat_it_id';
		";
