...
/**
 * v6 의 misMenuList_change → v7 에선 before_query(&$menu, ...) 로 포팅.
 * v6 의 g09 (= base_filter) 를 동적으로 설정해 본인 데이터만 보이게 제한.
 * gadmin 은 DB 에 'admin' 으로 저장돼 있어서 매핑 처리.
 */
function before_query(&$menu, &$fields, &$params) {
    $uid = (string)($GLOBALS['misSessionUserId'] ?? '');
    if ($uid === 'gadmin') $uid = 'admin';
    // 사용자 ID 안전 sanitize (영숫자 + _ - 만 허용 — base_filter 는 raw SQL 로 합쳐지므로 필수)
    $safeUid = preg_replace('/[^A-Za-z0-9_\-]/', '', $uid);
    if ($safeUid === '') return;
    $menu['base_filter'] = "table_m.it_10 = '" . $safeUid . "'";
}

// ── v6 misMenuList_change 원본 (참고용) — v7 에선 위 before_query() 가 동등 역할.
// 이 함수는 v7 에서 호출되지 않음.
