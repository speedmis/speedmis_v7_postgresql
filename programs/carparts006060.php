...
function list_json_init() {
    ...
    if($flag=='read') { 
        if($app=='자동정렬') {
            $appSql = "select count(idx) from parts_cate where sortG01='0' and use_yn=1 ";
            $cnt = onlyOnereturnSql($appSql);
            if($cnt==1) {
                if($MS_MJ_MY=='MY') {
                    $appSql = "call parts_cate_ordering_proc();";
                } else {
                    $appSql = "EXECUTE parts_cate_ordering_proc";
                }
                if(execSql($appSql)) {
                    $resultCode = "success";
...
//exit;
	...
	$autogubun = onlyOnereturnSql("select autogubun from parts_cate where idx='" . $updateList["upidx"] . "'");
