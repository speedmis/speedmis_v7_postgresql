...
function makeLink(userid,username) {
	...
	url = "addLogic_treat.php?real_pid=<?=$real_pid?>&userid="+userid+"&username="+username+"&expireday="+expireday;
...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=="virtual_fieldQnpwReset") {
			var rValue = `<a href="javascript:;" onclick="makeLink('` + p_dataItem["user_id"] + `','` + p_dataItem["user_name"] + `');" class='k-button'>링크생성</a>`;
...
function save_writeAfter() {
    ...
    global $base_root, $real_pid, $mis_join_pid, $logicPid, $parent_idx, $full_siteID;
