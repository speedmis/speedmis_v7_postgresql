...
function pop_phpCodePaste(p_field) {
    ...
    var r = ajax_url_return('https://www.speedmis.com/_mis/list_json.php?flag=read&real_pid=speedmis001040&$orderby=idx&recently=N&allFilter=[{"operator":"contains","value":"'+keyword+'","field":"zjemok"}]&$callback=jQuery112407752819346753541_1599384564376');
...
function pop_phpCodeExample(p_field) {
    if(p_field=='info') keyword = '상단 주석문'; else keyword = p_field;
    url = 'index.php?real_pid=speedmis001040&allFilter=[{"operator":"contains","value":"'+keyword+'","field":"zjemok"}]&isSplitter=Y';
...
function list_json_load() {
    global $base_root, $real_pid, $mis_join_pid, $logicPid, $parent_idx, $addDir;
