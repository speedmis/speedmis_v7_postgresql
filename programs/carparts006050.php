...
*/
	$sid = requestVB('sid');
	if($sid!='') {
		$sql = "SELECT full_name FROM v_parts_storage_tree  WHERE idx=$sid;";
		$full_name = onlyOnereturnSql($sql);
		if($full_name!='') {
			$url = 'index.php?gubun=6046&allFilter=[{"operator":"contains","value":"' . $full_name . '","field":"table_storage_cate_idxQnfull_name"}]&isMenuIn=Y';
			re_direct($url);
		} else {
?>
<script>
	alert('해당 창고번호: <?php echo $sid; ?> 는 존재하지 않습니다.');
	location.href = "index.php?gubun=6046&isMenuIn=Y";
</script>
<?php
		}
		exit;
...
function convertEngToKor(englishInput) {
    // 1. 영문 자판(Shift 미적용)과 한글 자판(Shift 미적용) 매핑 테이블
    const map = {
        'q': 'ㅂ', 'w': 'ㅈ', 'e': 'ㄷ', 'r': 'ㄱ', 't': 'ㅅ',
        'y': 'ㅛ', 'u': 'ㅕ', 'i': 'ㅑ', 'o': 'ㅐ', 'p': 'ㅔ',
        'a': 'ㅁ', 's': 'ㄴ', 'd': 'ㅇ', 'f': 'ㄹ', 'g': 'ㅎ',
        ...
        // 2. Shift 키 적용된 영문 자판과 한글 자판 매핑 테이블 (쌍자음/쌍모음/특수문자)
        'Q': 'ㅃ', 'W': 'ㅉ', 'E': 'ㄸ', 'R': 'ㄲ', 'T': 'ㅆ',
        'O': 'ㅒ', 'P': 'ㅖ'
    };
    ...
    for (let i = 0; i < englishInput.length; i++) {
        ...
        // map에 해당 키가 있으면 변환하고, 없으면 원래 문자 그대로 사용 (예: 스페이스바)
        if (map[char]) {
            converted += map[char];
        } else {
            converted += char;
...
function listLogic_afterLoad_once()	{
	...
	document.addEventListener('keydown', function(e) {
		...
		// 리더기에서 연속적으로 빠르게 입력되는 문자를 감지 (일반적인 타이핑보다 빠름)
		// 100ms 이내에 다음 키 입력이 들어오면 연속적인 입력으로 간주합니다.
		if (currentTime - lastTime > 1000) {
			qrCodeValue = ''; // 새 입력이 시작되면 버퍼 초기화
		}

		// Enter 키 또는 Tab 키 감지 (QR 코드 입력의 종료 신호)
		if (e.key === 'Enter' || e.key === 'Tab') {
			e.preventDefault(); // 기본 동작 방지 (특히 폼 제출 방지)

			if (qrCodeValue.length > 0) {
				console.log('QR 코드 값 수신:', qrCodeValue);
				
				// hangul-js 라이브러리가 로드되었다고 가정
				const koreanResult = convertEngToKor(qrCodeValue);
				console.log('koreanResult:', koreanResult);
				// 1. 입력 문자열을 개별 자모 배열로 분리
				// 이 과정은 입력 자체가 이미 분리된 자모이므로 필요 없을 수 있으나, 라이브러리의 표준 입력 형태를 따름.
				const inputArr = koreanResult.split(''); 

				// 2. 조합 함수 실행
				// assemble() 함수가 분리된 자모 배열을 받아서 완성형 한글로 조합합니다.
				const koreanResult2 = Hangul.assemble(inputArr);
				console.log('koreanResult2:', koreanResult2);

				console.log(koreanResult2);
				console.log('koreanResult2=', koreanResult2);
				
				if(InStr(koreanResult2,'.')>0 && isNumeric(Right(koreanResult2,1))) {
					koreanResult3 = replaceAll(koreanResult2,'.',' > ');
					url = 'index.php?gubun=6046&isMenuIn=Y&allFilter=[{"operator":"contains","value":"'+koreanResult3+'","field":"table_storage_cate_idxQnfull_name"}]&orderby=cpQnpart_cate_fullname,idx&recently=N&isAddURL=Y';
					go_mis_gubun(6046, url);
				}

				// TODO: 수신된 qrCodeValue ('포천|범1|단1|칸1')를 처리하는 로직을 여기에 구현합니다.

				qrCodeValue = ''; // 처리 후 초기화
			}
		} else {
			// 일반적인 문자/숫자 입력 (shift, ctrl, alt 등 특수키 제외)
			if (e.key.length === 1) {
				qrCodeValue += e.key;
			}
		}
	...
	// 사용자가 쉽게 볼 수 있도록 메시지를 표시
	console.log('QR 코드 리더기 입력을 기다리는 중...');	
...
function list_query() {
    ...
    if($flag=='read') { 
        //$countQuery = replace($countQuery, "and table_m.car_parts_id = '$parent_idx'", "and cp.part_cate_fullname LIKE CONCAT((SELECT full_name FROM v_parts_cate_tree WHERE idx = (SELECT part_cate_idx FROM car_parts WHERE id=$parent_idx)),'%')");
$countQuery = 'select 1 from dual';        
$selectQuery = replace($selectQuery, "and table_m.car_parts_id = '$parent_idx'", "and cp.part_cate_fullname LIKE CONCAT((SELECT full_name FROM v_parts_cate_tree WHERE idx = (SELECT part_cate_idx FROM car_parts WHERE id=$parent_idx)),'%')");
...
INSERT INTO parts_storage_mapping (car_parts_id, storage_cate_idx)
SELECT
...
WHERE
    NOT EXISTS (
        SELECT 1
        FROM parts_storage_mapping AS mpsm
...
delete from parts_storage_mapping where car_parts_id=$car_parts_id and idx=$idx
