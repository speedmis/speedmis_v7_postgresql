...
function pageLoad() {
    ...
    if($ActionFlag=="list" || $ActionFlag=="view") {
        ...
        $sql = "
        update mis_comments set sel_like=0 where sel_like is null;
        update mis_comments set sel_hate=0 where sel_hate is null;
        ";
...
function save_updateBefore() {
    ...
    if($ActionFlag=="write") {
		...
		$tempSql = "select case when menu_type='01' then g08 else (select m.g08 from mis_menus m where m.RealPid=table_m.MisJoinPid) end 
		from mis_menus table_m where RealPid='". $updateList["RealPid"] . "'";
...
function save_writeBefore() {
    ...
    if($MS_MJ_MY=='MY') {
        $sql = "select ifnull(dbalias,'') from mis_menus where RealPid='" . get_logicPid($parent_idx) . "'";
    } else {
        $sql = "select isnull(dbalias,'') from mis_menus where RealPid='" . get_logicPid(GubunIntoRealPid($parent_gubun)) . "'";
    }
    ...
    if($MS_MJ_MY=='MY') {
        //검증필요
        $parent_field = onlyOnereturnSql("select db_field from mis_menu_fields where RealPid=
        (select case when menu_type='01' then RealPid else MisJoinPid end from mis_menus 
        where RealPid='" . $updateList["RealPid"] . "')
        and (sort_order=1 and col_width>-1 or sort_order>=2) order by sort_order limit 1");

        $sql_next = " 
        update mis_comments set refidx=idx where idx=$newIdx or refidx is null;
        ";
        if($parent_MS_MJ_MY=='MY') $sql_next = $sql_next . " 
        select @cnt=count(*) from mis_comments 
        ...
        $parent_field = onlyOnereturnSql("select top 1 db_field from mis_menu_fields where RealPid=
        (select case when menu_type='01' then RealPid else MisJoinPid end from mis_menus 
        where RealPid='" . $updateList["RealPid"] . "')
        and (sort_order=1 and col_width>-1 or sort_order>=2) order by sort_order");

        $sql_next = " 
        update mis_comments set refidx=idx where idx=$newIdx or refidx is null
        ";
        if($parent_MS_MJ_MY=='MS' || $parent_MS_MJ_MY=='MJ') $sql_next = $sql_next . " 
        declare @cnt int select @cnt=count(*) from mis_comments 
        where RealPid='" . $updateList["RealPid"] . "' and midx='" . $updateList["midx"] . "' and useflag=1 
...
function save_writeAfter() {
    ...
    global $default_value, $ActionFlag, $MisSession_UserID, $newIdx;
    global $afterScript;
...
function save_deleteBefore() {
    ...
    if($MS_MJ_MY=='MY') {
        ...
        and col_width>-1 order by sort_order limit 1");

        $parent_table = onlyOnereturnSql("select g08 from mis_menus where RealPid=
        (select case when menu_type='01' then RealPid else MisJoinPid end from mis_menus 
        where RealPid='" . GubunIntoRealPid($parent_gubun) . "')");

        $sql_next = " 
        update mis_comments set useflag=0 from mis_comments m, (
            select refidx from mis_comments where refidx in 
            (select refidx from mis_comments where idx=refidx and useflag=0 and concat(midx,RealPid)=(select concat(midx,RealPid) 
            from mis_comments where idx in ($deleteList)))
            ) d where m.refidx=d.refidx;

        select @cnt=COUNT(*) from mis_comments where useflag=1 and midx = (
         select midx from  mis_comments  where idx in ($deleteList));
        update $parent_table set comment_count=@cnt where $parent_field=(select midx from mis_comments where idx in ($deleteList));
        ";
    } else {
        $parent_field = onlyOnereturnSql("select top 1 db_field from mis_menu_fields where RealPid=
        (select case when menu_type='01' then RealPid else MisJoinPid end from mis_menus 
        where RealPid='" . GubunIntoRealPid($parent_gubun) . "')
        and col_width>-1 order by sort_order");
        ...
        $sql_next = " 
		update mis_comments set useflag=0 from mis_comments m join (
            select refidx from mis_comments where refidx in 
            (select refidx from mis_comments where idx=refidx and useflag=0 and convert(varchar(5),midx)+RealPid=(select convert(varchar(5),midx)+RealPid 
            from mis_comments where idx in ($deleteList)))
            ) d on m.refidx=d.refidx where m.refidx=d.refidx
            
        declare @cnt int select @cnt=COUNT(*) from mis_comments where useflag=1 and midx = (
         select midx from  mis_comments  where idx in ($deleteList))
        update $parent_table set comment_count=@cnt where $parent_field=(select midx from mis_comments where idx in ($deleteList))
        ";
