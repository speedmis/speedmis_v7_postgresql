...
class MenuRouter
{
    ...
    public function getMenu(int $gubun): array
    {
        try {
            ...
            $stmt->execute([$gubun]);
            $menu = $stmt->fetch() ?: [];
            if (isset($menu['add_url'])) $menu['add_url'] = self::resolveAddUrlPlaceholders($menu['add_url']);
    ...
    /**
     * add_url 의 동적 날짜 placeholder 치환 (3 paths 공통: tree/getMenu/getMenuByRealPid)
     * 지원: @today / @thisMonth / @prevMonth (=@lastMonth) / @nextMonth / @thisYear
     *       $ 접두사도 동일하게 동작 (v7 표준)
     * 주의 1: strtotime('-1 month') 는 31일 등에서 롤오버 버그 (5/31 → 5/1)
     *         'first day of this month' 후 ±1 month 가 안전.
     * 주의 2: 서버가 UTC 면 한국 자정 직후 9시간 동안 어제 날짜 → Asia/Seoul 강제.
     */
    public static function resolveAddUrlPlaceholders(string $addUrl): string
    {
        if ($addUrl === '' || (!str_contains($addUrl, '@') && !str_contains($addUrl, '$'))) {
            return $addUrl;
        }
        ...
        $prevMonth = (clone $now)->modify('first day of this month')->modify('-1 month')->format('Y-m');
        $nextMonth = (clone $now)->modify('first day of this month')->modify('+1 month')->format('Y-m');
        return str_replace(
    ...
    private function fetchMenusByAuth(string $userId): array
    {
        try {
            ...
            foreach ($rows as &$r) {
                $r['add_url'] = self::resolveAddUrlPlaceholders($r['add_url'] ?? '');
            }
            unset($r);
            return $rows;
        } catch (\Throwable $e) {
